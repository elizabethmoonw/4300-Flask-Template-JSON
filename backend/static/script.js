...
function filterText(){
  ...
  .then((data) => data.forEach(row => {
      ...
      tempDiv.innerHTML = answerBoxTemplate(row.product,row.category,row.link,row.price)
...
async function matchProducts() {
  var match = ''
  await fetch("/search?" + new URLSearchParams({ title: document.getElementById("prod-search-text").value }).toString())
  .then((response) => response.json())
  .then((data) => data.forEach(row => {
